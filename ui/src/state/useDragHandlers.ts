...
type PatchFn = (
  entityId: string,
  update: UpdateEntityDisplayDto,
) => Promise<unknown>;
type ReorderFavsFn = (
  items: { entity_id: string; favorite_order: number }[],
) => Promise<unknown>;
type ReorderRoomEntsFn = (
  items: { entity_id: string; sort_order: number }[],
) => Promise<unknown>;
...
export function useDragHandlers({
      ...
      if (src === dst) {
        ...
        if (src === FAVORITES_CONTAINER_ID) {
          ...
          void reorderFavoritesOptimistic(
            list.map((e, i) => ({ entity_id: e.entity_id, favorite_order: i })),
          );
          return;
        }
        if (src.startsWith("room:")) {
          const list = (entitiesByRoom.get(src.slice(5)) ?? [])
            .slice()
            .sort(bySortOrder);
          const fi = list.findIndex((e) => e.entity_id === activeId);
          const ti = list.findIndex((e) => e.entity_id === overId);
          ...
          void reorderRoomEntitiesOptimistic(
            list.map((e, i) => ({ entity_id: e.entity_id, sort_order: i })),
          );
        }
        return;
      }
      if (dst === FAVORITES_CONTAINER_ID) {
  ...
  const handleSectionDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      void reorderRooms(
        instanceId,
        ordered.map((id, i) => ({ room_id: id, sort_order: i })),
      );
    },
    [rooms, instanceId],
  );
